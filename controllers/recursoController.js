...
exports.actualizarRecurso = async (req, res) => {
  try {
    ...
    vrecurso = await Recurso.findOneAndUpdate(
      { _id: req.params.id },
      vrecurso,
      { new: true }
    );
...
exports.asignarRecurso = async (req, res) => {
  ...
  try {
    ...
    requestBody.recursos.forEach(async (f) => {
      ...
      await Recurso.findOneAndUpdate({ _id: recurso._id }, recurso, {
        new: true,
      });
    });
...
exports.reportarFallas = async (req, res) => {
  ...
  try {
    const requestBody = req.body;
    console.info(requestBody);
    let vrecurso = await Recurso.findOne({
      numSerie: { $eq: requestBody.numSerie },
    });
    ...
    vrecurso = await Recurso.findOneAndUpdate({ _id: vrecurso._id }, vrecurso, {
      new: true,
    });
    res.send({ mensaje: "Actualizado correctamente" });
  } catch (error) {
